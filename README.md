...
`aipick` launches Aider or Qwen Code against a provider from `~/.config/aipick/providers.json` (nvidia, openrouter free tier, a LAN GPU box, local ollama); `orcheck` flags openrouter models that are no longer free. Claude Code itself always talks directly to Anthropic — the CCR proxy that used to sit in front of it was removed, since it carried no routing rules and turned any router crash into a total outage.
