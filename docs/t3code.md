...
would add reachable surface without adding reach.
...
Bind it to the tailnet address and let T3 drive `tailscale serve` for TLS:

```bash
npx t3@latest serve --host "$(tailscale ip -4)" --tailscale-serve
```

The server is then at `https://devbox.<tailnet>.ts.net/`.
...
Prints a one-time token, a pairing URL, and a QR code. Scan it on the phone.
